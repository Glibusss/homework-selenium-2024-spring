...
class CampaignPage(BasePage):
    ...
    SECTION_NAME_LIST = ['Регионы показа', 'Демография', 'Интересы и поведение', 'Устройства']
    ...
    AD_BUTTON_TEXT = 'GOOOAL'
    AD_ADVERTISER = 'BMSTU on Yauza City, Mr. Makarenko'
    ...
    def budget_inputs_became_visible(self):
        return self.became_visible(self.locators.BUDGET_INPUT) and self.became_visible(self.locators.BUDGET_PERIOD_SELECT)
    ...
    def fill_campaign_form(self):
        ...
        #С селектами такой прикол, что я нашел только такой метод
        self.click(self.locators.DROPDOWN_OPTIONS(self.CAMPAIGN_ACTION))
        self.click(self.locators.STRATEGY_ACTION_SELECT)
        #Приходится тыкать потому что с 1 раза гарантированное не работает
        self.click(self.locators.CREATE_FOOTER_CONTINUE)
    ...
    def has_age_select_content(self):
        return self.became_visible(self.locators.AGE_SELECT_CHECK('12')) and self.became_visible(self.locators.AGE_SELECT_CHECK('75'))
    ...
    def has_interest_subsection_content(self):
        return self.became_visible(self.locators.INTEREST_SELECT) and self.became_visible(self.locators.STOP_INTEREST_OPENER)
    ...
    def has_device_section_content(self):
        return self.became_visible(self.locators.DESKTOP_CHECKBOX) and self.became_visible(self.locators.MOBILE_CHECKBOX)
    ...
    def has_ads_inputs_content(self):
        return self.became_visible(self.locators.TITLE_INPUT) and self.became_visible(self.locators.BUTTON_TEXT_INPUT)
    
    def has_ads_textarea_content(self):
        return self.became_visible(self.locators.SHORT_DESCRIPTION) and self.became_visible(self.locators.LONG_DESCRIPTION) and self.became_visible(self.locators.ADVERTISER_DESCRIPTION)
    ...
    def fill_ad_inputs_and_textarea(self):
        ...
        short = self.find(self.locators.SHORT_DESCRIPTION)
        short.clear()
        short.send_keys(self.AD_SHORT_DESC)
        long = self.find(self.locators.LONG_DESCRIPTION)
        long.clear()
        ...
        advertiser = self.find(self.locators.ADVERTISER_DESCRIPTION)
        advertiser.clear()
    ...
    def buttons_changed(self):
        return self.became_invisible(self.locators.CREATE_FOOTER_CONTINUE) and self.became_visible(self.locators.CREATE_FOOTER_PUBLISH)
    
    def has_bug_modal_content(self):
        return self.became_visible(self.locators.MODAL_CANCEL_BUTTON) and self.became_visible(self.locators.MODAL_SEND_BUTTON)
    ...
    def check_site_name_value_campaign(self):
        return self.became_visible(self.locators.SIDEBAR_SITE_NAME_INPUT(self.URL))
    ...
    def check_ages(self):
        return self.became_visible(self.locators.AGE_SELECT_CHECK(self.MIN_AGE)) and self.became_visible(self.locators.AGE_SELECT_CHECK(self.MAX_AGE))
    ...
    def check_devices(self):
        return self.became_visible(self.locators.CHECKBOX_NAME_IS_CHECKED_OR_NOT('Десктопные', 'on')) and self.became_visible(self.locators.CHECKBOX_NAME_IS_CHECKED_OR_NOT('Мобильные', 'off'))
    ...
    def check_ad_short_description(self):
        return self.find(self.locators.SHORT_DESCRIPTION).text == self.AD_SHORT_DESC
    
    def check_ad_long_description(self):
        return self.find(self.locators.LONG_DESCRIPTION).text == self.AD_LONG_DESC
