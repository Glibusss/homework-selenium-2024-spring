...
from ui.locators.settings_notifications_page_locators import (
    SettingsNotificationsPageLocators,
)
...
class SettingsNotificationsPage(BasePage):
    ...
    def has_switch_email_content(self):
        return self.became_visible(
            self.locators.SWITCH_EMAIL_SIGN
        ) and self.became_visible(self.locators.SWITCH_EMAIL_INPUT)

    def has_telegram_cell_content(self):
        return self.became_visible(self.locators.TELEGRAM_SIGN) and self.became_visible(
            self.locators.TELEGRAM_BUTTON
        )

    def has_save_modal_content(self):
        return (
            self.became_visible(self.locators.SAVE_MODAL)
            and self.became_visible(self.locators.SAVE_MODAL_CANCEL)
            and self.became_visible(self.locators.SAVE_MODAL_SAVE)
        )
    ...
    def checkbox_not_checked(self):
        return self.became_visible(
            self.locators.CHECKBOX_NAME_IS_CHECKED_OR_NOT(self.CHECKBOX_LIST[2], "off")
        )
