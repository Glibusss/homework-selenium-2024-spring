...
class StudyingPage(BasePage):
    ...
    PR_OBJECTS_LIST = ['Сообщество ВКонтакте', 'Каталог товаров', 'Лид-формы', 'Музыка', 'Дзен', 'Сайт', 'Мобильное приложение', 'VK Mini Apps', 'Видео и трансляции']
    PR_METHODS_LIST = ['Создать каталог с подсказками', 'Смотреть видеоурок от экспертов VK', 'Смотреть курс на обучающей платформе']
    ...
    def tips_became_visible(self):
        return self.became_visible(self.locators.TIP) and self.became_visible(self.locators.SIDEBAR) and self.became_visible(self.locators.TIP_CLOSE)
    ...
    def stop_onboarding_modal_buttons_became_visible(self):
        return self.became_visible(self.locators.BREAK_BUTTON) and self.became_visible(self.locators.CANCEL_BUTTON)
