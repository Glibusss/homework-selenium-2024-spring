...
class SettingsNotificationsPageLocators(BasePageLocators):
    ...
    def H2_NAME(h2):
        return (
            By.XPATH,
            f"//*[contains(@class, 'vkuiTitle--level-2') and text()='{h2}']",
        )
    ...
    def CHECKBOX_NAME(checkbox):
        return (
            By.XPATH,
            f"//*[contains(@class, 'Subscriptions_checkboxContent__') and text()='{checkbox}']",
        )
    ...
    SWITCH_EMAIL_INPUT = (
        By.XPATH,
        "//*[contains(@class, 'vkuiSimpleCell')]//label[contains(@class, 'vkuiSwitch')]",
    )

    TELEGRAM_SIGN = (
        By.XPATH,
        "//*[contains(@class, 'Telegram_title__') and text()='Сообщение в Telegram']",
    )
    TELEGRAM_BUTTON = (
        By.XPATH,
        "//*[contains(@class, 'vkuiButton__content') and text()='Подключить']",
    )
